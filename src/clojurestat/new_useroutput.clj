...
(defn userOutputFromLetters
  [inputRegex systemLetter]
  ;systemmap wubilettertopair
  ;(get (get inputMethodEachLineMap systemLetter) "lettertopair")
  ((comp
     (fn [inputlist]
       (map
         (fn [eachLettercharPair]
           (if (get talmap (str (second eachLettercharPair)))
             (merge-with + (hash-map :code (vector systemLetter (sort compareLength
                                                                      (get
                                                                        ;wubichartoletters
                                                                        (get (get inputMethodEachLineMap systemLetter) "chartoletters")
                                                                        (second eachLettercharPair)))));(first eachLettercharPair)))
                         (get talmap (str (second eachLettercharPair))))
             (hash-map
               :code (vector systemLetter (sort compareLength (get
                                                                ;wubichartoletters
                                                                (get (get inputMethodEachLineMap systemLetter) "chartoletters")
                                                                (second eachLettercharPair))));(first eachLettercharPair))
     ...
     #(partition 2 %)
     flatten
     #(map (get (get inputMethodEachLineMap systemLetter) "lettertopair") %)
     regexInputSystem
     ) (get (get inputMethodEachLineMap systemLetter) "lettertopair") inputRegex))
...
(println "dett er new useroutput")
